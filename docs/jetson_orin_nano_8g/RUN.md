...
| Flag | Purpose |
|------|---------|
| `--device /dev/dri` | Passes the Jetson iGPU render node into the container |
...
> **Note:** The Jetson Orin Nano 8G has 8 GB of unified memory shared between CPU and GPU. With the Q4_K_M quantization (~800 MB model), all layers fit in GPU memory comfortably.
